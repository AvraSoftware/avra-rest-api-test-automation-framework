package com.avra.qa.common.util.datautil.postgres;
...
public class QueryHelper {
    ...
    public List<Map<String, String>> getQueryResultAsListMap(String sql, MapSqlParameterSource params) {
        List<Map<String, Object>> result = namedParameterJdbcTemplate.queryForList(sql, params);
        return result.stream()
                .map(row -> row.entrySet().stream().collect(Collectors.toMap(Map.Entry::getKey, valueEntry -> String.valueOf(valueEntry.getValue()))))
    ...
    public <T> T getQueryResultByResultSetExtractor(String sql, ResultSetExtractor<T> extractor) {
        ResultSetExtractor<T> wrapper = rs -> {
            if (rs.next()) {
                return extractor.extractData(rs);
            } else {
    ...
    public List<String> getQueryResultLimitedToColumn(String sql, String columnName) {
        RowMapper<String> rowMapper = (rs, rowNum) -> rs.getObject(columnName).toString();
        return namedParameterJdbcTemplate.query(sql, rowMapper);
    }

    public List<Long> getQueryResultFromCount(String sql) {
        RowMapper<Long> rowMapper = (rs, rowNum) -> (Long) rs.getObject("count");
        return namedParameterJdbcTemplate.query(sql, rowMapper);
    }

    public List<Timestamp> getQueryResultLimitedToColumnAsTimestamps(String sql, String columnName) throws SQLException {
        RowMapper<Timestamp> rowMapper = (rs, rowNum) -> rs.getTimestamp(columnName);
        return namedParameterJdbcTemplate.query(sql, rowMapper);
    }

    public void waitForDatabaseRow(String sqlTable, String columnIdName, String id) {
        String selectSql = "SELECT count(*) FROM " + sqlTable + " WHERE " + columnIdName + "='" + id + "';";
        try {
            await()
                    .atMost(timeout, TimeUnit.MILLISECONDS)
                    .pollInterval(15, TimeUnit.MILLISECONDS)
                    .until(() -> getQueryResultByResultSetExtractor(selectSql, rs -> rs.getInt(1)) > 0);
    ...
    public void waitForDatabaseUpdate(String sqlTable, String rowIdentifier, String columnName, String expectedValue) {
        ...
        try {
            await()
                    .atMost(timeout, TimeUnit.MILLISECONDS)
                    .pollInterval(15, TimeUnit.MILLISECONDS)
                    .until(() -> getQueryResultLimitedToColumn(selectSql, columnName).get(0).contains(expectedValue));
        } catch (ConditionTimeoutException e) {
            throw new RuntimeException("Database row not updated: table: " + sqlTable + ", identifier: " + rowIdentifier + ", column:" + columnName
                    + ", expectedValue:" + expectedValue + ", actualValue: " + getQueryResultLimitedToColumn(selectSql, columnName).get(0));
        }
    }

    public void waitForDatabaseRowWhereColumnIsEqualToValue(String sqlTable, String columnIdName, Integer id) {
        String selectSql = "SELECT count(*) FROM " + sqlTable + " WHERE " + columnIdName + " = :id;";
